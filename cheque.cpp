...
QSqlQueryModel * Cheque::chercher(QString rech)
{
QSqlQueryModel * model6 = new QSqlQueryModel();
model6->setQuery("SELECT * FROM CHEQUE WHERE BANQUE LIKE'%"+rech+"%' or MONTANT LIKE'%"+rech+"%' or NUM_CHEQUE LIKE'%"+rech+"%' or DATE_ECHEANCE LIKE'%"+rech+"%' or ID_CLIENT LIKE'%"+rech+"%' ");
return model6;
}
